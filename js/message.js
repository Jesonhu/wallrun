// $(function() {
    ...
    var pageNum = 0; // ajax请求的页码
    var nowItmes; // 当前items总数即总共的签到人数
    var onePageLen = 3; // 每页页码的数量3（每页最多可以显示的数量）
    var nowResCount; // 当前页ajax返回了多少条数据
    ...
    setInterval(function() {
        if (pageNum == 0) { // 第一次ajax请求
            ...
            $.ajax({
                ...
                success: function(resData) {
                    if (resData.code === 1) {
                        ...
                        innerRenderDom(resData.list);
                        function innerRenderDom(arr) {
                            let lis = '';
            ...
            let integers = Math.floor( nowItmes / onePageLen );
                ...
                $.ajax({
                    ...
                    success: function(resData) {
                        if (resData.code === 1) {
                            ...
                            // 判断当前返回的数据是否大于当前页的数量 
                            // 每次返回的数量 [0, 18]
                            // 当前页显示的数量  nowItmes / pageNum ==> wrong ; nowItmes是显示的item总数 而且结果可能为小数

                            // resCountMoreLen = (nowResCount - (nowItmes / pageNum)); => wrong公式错误
                            ...
                            if (!isResCountMoreSymbol) { // 如果当前返回的数量不大于当前页显示的数量 => 直接返回不再渲染dom
                                ...
                                // 当前 [1, 2]
                                // 返回 [1, 2, 3]
                                // let nowIndex = (nowItmes / pageNum); // 当前显示的item数量在每页18条中的索引+1 => wrong 公式错误
